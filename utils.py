...
from transformers import AutoModelForSeq2SeqLM, AutoTokenizer, T5Config, set_seed
...
def get_dataset(
    ...
    if (
        args.datafile_path == "NicolaiSivesind/ChatGPT-Research-Abstracts"
    ):  # max_length = 455
        ...
        if os.path.exists("./nlp_dataset/ChatGPT-Research-Abstracts/index_train.txt"):
            with open(
                "./nlp_dataset/ChatGPT-Research-Abstracts/index_train.txt", "r"
            ) as f:
            ...
            with open(
                "./nlp_dataset/ChatGPT-Research-Abstracts/index_train.txt", "w"
            ) as f:
        ...
        dataset_df["chatgpt_answers"] = dataset_df["chatgpt_answers"].apply(
            lambda x: x[0]
        )
        ...
        args.datafile_path == "NicolaiSivesind/ChatGPT-Research-Abstracts-human"
    ):  # max=584/ token=590
        dataset = load_dataset("NicolaiSivesind/ChatGPT-Research-Abstracts")
        # filter dataset with word count > 100
        dataset_df = dataset["train"].to_pandas()
        ...
        if os.path.exists(
            "./nlp_dataset/ChatGPT-Research-Abstracts-human/index_train.txt"
        ):
            with open(
            ...
            # save index_train to file
            with open(
                "./nlp_dataset/ChatGPT-Research-Abstracts-human/index_train.txt", "w"
            ) as f:
                f.write(",".join([str(i) for i in index_train]))
    ...
    def preprocess_function(sample, padding="max_length"):
        ...
        if args.target_text_type == "original":
            ...
            input_ids = torch.cat(
                [
                    input_ids,
                    torch.tensor([[tokenizer.pad_token_id]]).repeat(
                        input_ids.shape[0], 1
                    ),
                ],
                dim=1,
            ...
            for c in column:
                ...
                total_label = torch.cat(
                    [total_label, torch.tensor(labels["input_ids"])], dim=1
                )
    ...
    def str_convert(example):
        ...
        if args.figurepint:
            message = numpy.random.randint(
                0, 2, size=(int(args.message_max_length - 4))
            )
            ...
            if example["generated_word_count"] > 400:
                ...
                example["generated_word_count"] <= 400
                and example["generated_word_count"] > 200
            ):
