...
from utils import get_dataset, log_info, modify_probe
...
def parse_args():
    ...
    parser.add_argument(
        "--datafile_path",
        type=str,
        default="Hello-SimpleAI/HC3-gpt",
        help="dataset name",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Maximum message length to use for generation",
    )
    parser.add_argument(
        "--target_max_length",
        type=int,
    ...
    parser.add_argument(
        "--lr", type=float, default=3e-4, help="Learning rate to use for training."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train_subset", type=int, default=0, help="Train subset of dataset"
    )
    parser.add_argument(
        "--train_semantic", type=int, default=0, help="Train semantic loss"
    )
    parser.add_argument(
        "--train_semantic_loss",
        type=str,
        default="cosine",
        help="semantic backward loss",
    )
    parser.add_argument(
        "--wm_embed_model", type=str, default="t5", help="model to insert wm"
    )  # add wm model to t5/lora
    ...
    parser.add_argument(
        ...
        type=str,
        default="original",
        help="which text is used for target text",
    )
    parser.add_argument(
        "--inference_strategy",
        type=str,
        default="distribution",
        help="use distribution or token id to inferece",
    )
    parser.add_argument(
        "--mapper_info",
        type=str,
        default="logits",
        help="use logits or embedding to map",
    )
    parser.add_argument(
        "--inference_batch", type=int, default=0, help="inference batch or per"
    )
    parser.add_argument(
        "--attack", type=int, default=0, help="test attack accuray or not"
    )
    parser.add_argument(
        "--train_rephrase", type=int, default=0, help="train with rephrase or not"
    )
    parser.add_argument(
        "--train_attack", type=int, default=1, help="train with attack sample or not"
    )
    parser.add_argument(
        "--schedule_tmp", type=int, default=0, help="schedule tmp or not"
    )
    parser.add_argument(
        "--figurepint", type=int, default=0, help="figurepint the texts or not"
    )
    parser.add_argument(
        "--periodical", type=int, default=0, help="periodical the texts or not"
    )
    parser.add_argument(
        "--discriminator", type=int, default=0, help="malicious transformation"
    )
    parser.add_argument(
        "--augument_train", type=int, default=0, help="use synoym for augument training"
    )
    parser.add_argument("--adaptive", type=int, default=0, help="adaptive set message")
    parser.add_argument(
        "--message_embed_method",
        type=str,
        default="concate",
        help="way to embed message",
    )
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch #{epoch} of {num_epochs}")):
            message_base, message_all, input_ids_original, attention_mask, labels = (
                batch["message_base"],
                batch["message_all"],
                batch["input_ids"],
            ...
            message_base, message_all, input_ids_original, attention_mask, labels = (
                message_base.to(device),
                message_all.to(device),
                input_ids_original.to(device),
            ...
            if args.wm:
                outputs = model(
                    ...
                    labels=labels,
                    message=message_all,
                    message_embed_method=args.message_embed_method,
                )  # [8, 512, 32128] (batch_size, seq_len, vocab_size)
            else:
                outputs = model(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
            ...
            probs = torch.nn.functional.gumbel_softmax(
                output_logits, tau=temp, hard=False, dim=-1
            )
            ...
            if train_semantic and args.train_semantic_loss == "cosine":
                ...
                if args.wm and epoch >= step_ep:
                    if args.discriminator and epoch >= adv_ep:
                        ...
                        total_loss_back = (
                            0.7 * extractor_loss + 0.3 * adve_loss + outputs.loss
                        )
                    else:
            ...
            if step == 0 and args.visualize:
                ...
                sentence = tokenizer.batch_decode(
                    input_ids_original, skip_special_tokens=True
                )
        ...
        log_info(
            logging,
            f"epoch: {epoch} num_layer: {num_layers} cur_tmp: {temp} cur_mask: {maskper} cur_acc: {cur_acc} best_acc: {best_acc} train_message_loss: {train_message_loss} total_cos_loss: {total_cos_loss} train_epoch_loss: {train_epoch_loss}",
        )
