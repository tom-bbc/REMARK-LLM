...
class Embedding_mapping(nn.Module):
    def __init__(
        self, d_model, nhead, num_layers, dim_feedforward, num_classes, extract_layer
    ):
...
if torch.cuda.is_available():
    paraphrasing_model = paraphrasing_model.to("cuda")
...
def generate_paraphrases(sentence, top_k=120, top_p=0.9, device="cuda", max_length=768):
    ...
    encoding = paraphrasing_tokenizer.encode_plus(
        text, max_length=256, padding="max_length", return_tensors="pt"
    )
