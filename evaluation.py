...
def parse_args():
    ...
    parser.add_argument(
        "--dataset", type=str, default="NicolaiSivesind/ChatGPT-Research-Abstracts"
    )
    parser.add_argument(
        "--extract_layer", type=int, default=-1, help="layers to extract wm"
    )
    parser.add_argument(
        "--message_max_length",
        type=int,
    ...
    parser.add_argument(
        "--wm_embed_model", type=str, default="t5", help="wm embed model"
    )
...
def beam_search_with_gumbel(
    ...
    ]  # 0.0 here since we'll be summing log probs
...
def get_sample(model, input_ids, logits, attention, noise, sentence=2):
    ...
    for i in range(sentence):
        model_output = beam_search_with_gumbel(
            model, input_ids, logits, length=length, beam_width=5, temperature=noise
        )
...
def inference(
    ...
    for step, batch in enumerate(tqdm(val_dataloader)):
        ...
        input_dist = F.one_hot(
            input_ids_original, num_classes=config.vocab_size
        ).float()
        with torch.no_grad():
        ...
        while best_acc < 1:
            ...
            with torch.no_grad():
                ids = model(
                    ...
                    labels=input_ids_original,
                    attention_mask=attention_mask,
                )
            logits = ids.logits
            noise_idx = c % len(noise_list)

            token_ids = get_sample(
                model, input_ids, logits, attention_mask, noise=noise_list[int(c / 5)]
            )
            ...
            decode_token_test = token_ids[
                match_bit_list.index(max(match_bit_list))
            ].unsqueeze(0)
            decode_token_test = tokenizer.decode(
                decode_token_test[0][decode_token_test[0] != 0],
        ...
        decode_token = tokenizer.decode(
            decode_token[0][decode_token[0] != 0], skip_special_tokens=True
        )
        ...
        if len(total_bert_score) > 0:
            print(
                ...
                sum(blue4) / len(blue4),
                "Bert score: ",
                sum(total_bert_score) / len(total_bert_score),
            )
        blue4.append(blue4_score)
        # get bert score
        bert_score = bertscore.compute(
            predictions=[decode_token], references=[input_token], lang="en"
        )
...
def main():
    args, _ = parse_args()
    print(f"Args: {json.dumps(vars(args), indent=4)}")
    ...
    if args.dataset == "wiki":
        ...
        if os.path.isfile("./nlp_dataset/ChatGPT-Research-Abstracts/index_train.txt"):
        ...
        dataset_df = dataset["train"].to_pandas()
        dataset_df["mask"] = dataset_df.apply(
            lambda x: len(x["chatgpt_answers"]) > 0, axis=1
        )
        ...
        if os.path.isfile("./nlp_dataset/HC3-gpt/index_train.txt"):
        ...
        if os.path.isfile("./nlp_dataset/ChatGPT-Research-Abstracts-human/index_train.txt"):
            with open(
                "./nlp_dataset/ChatGPT-Research-Abstracts-human/index_train.txt", "r"
            ) as f:
                index_train = f.read().split(",")
                index_train = [int(i) for i in index_train]
        else:
    ...
    model_ckpt_path = os.path.join(args.load_path, "model.pt")
    model = AutoModelForSeq2SeqLM.from_pretrained(model_name_or_path, config=config)
    model.load_state_dict(torch.load(model_ckpt_path, map_location=device))

    extractor_ckpt_path = os.path.join(args.load_path, "extractor.pt")
    ...
    mapper_ckpt_path = os.path.join(args.load_path, "mapper.pt")
    ...
    def preprocess_function(sample, padding="max_length"):
        ...
        model_inputs = tokenizer(
            inputs, max_length=source_max_length, padding=padding, truncation=True
        )
        labels = tokenizer(
            text_target=sample[label_column],
